...
import serviceClosedDays from '../../../services/master/serviceClosedDays';
import { useDispatch, useSelector } from 'react-redux';
import { disableRefetch } from '../../../redux/slices/menu';
import { CheckOutlined, CloseOutlined } from '@ant-design/icons';
import { weeks } from '../../../components/shop/weeks';
import serviceWorkingDays from '../../../services/master/serviceWorkingDays.js';
...
const MasterClosedDays = ({}) => {
  ...
  const fetchClosedDays = () =>
    serviceClosedDays.getAll().then((res) => {
      const parsedDays = res.data.map((item) => new Date(item.date));
      setDays(parsedDays);
    });
  const fetchWorkingDays = () => {
    return serviceWorkingDays.getAll().then((res) => {
      setLines(
  ...
  const onFinish = (values) => {
    ...
    Promise.all([
      serviceClosedDays.update(userId, closedDaysParam),
      serviceWorkingDays.update(userId, workingDaysBody),
    ])
      .then((res) => {
  ...
  return (
    <Card title={t('master.closed.days')} loading={loading}>
